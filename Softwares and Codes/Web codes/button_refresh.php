...
//This page notifies the scheduling ON/OFF page (select.php) about the Current button status,feedback obtained,Scheduled commands.
    ...
    die("Connection failed: " . $conn->connect_error);
					...
					if ($result->num_rows > 0) {
						...
						while($row = $result->fetch_assoc()) 
						{
						   ...
						   $jsonitem['button1'] = $row["button"];  				//Creating name value pairs.
						   $jsonitem['button2'] = $row["button_2"];				// from table 'power'
					...
					$sql = "SELECT Button_change,Switch_off,Switch_on			
						   FROM scheduled";										//Change the table and selection of defined columns.
					 $g=1;														//$g is the index in the selected row, at $g the defined operation is carried 
					$result = $conn->query($sql);while($row = $result->fetch_assoc()) // out using first row of selected columns.
						{ 		
					    ...
					    else if($g==2)															//Second row in selected columns
					    ...
					    else if($g==3)															//third row in selected columns
						{
					...
					array_push($jsonarray,$jsonitem);
					echo json_encode($jsonarray);								//echo in JSON format
